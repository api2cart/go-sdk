/*
API2Cart OpenAPI

API2Cart

API version: 1.1
Contact: contact@api2cart.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the OrderShipmentInfo200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &OrderShipmentInfo200Response{}

// OrderShipmentInfo200Response struct for OrderShipmentInfo200Response
type OrderShipmentInfo200Response struct {
	ReturnCode *int32 `json:"return_code,omitempty"`
	ReturnMessage *string `json:"return_message,omitempty"`
	Result *Shipment `json:"result,omitempty"`
}

// NewOrderShipmentInfo200Response instantiates a new OrderShipmentInfo200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewOrderShipmentInfo200Response() *OrderShipmentInfo200Response {
	this := OrderShipmentInfo200Response{}
	return &this
}

// NewOrderShipmentInfo200ResponseWithDefaults instantiates a new OrderShipmentInfo200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewOrderShipmentInfo200ResponseWithDefaults() *OrderShipmentInfo200Response {
	this := OrderShipmentInfo200Response{}
	return &this
}

// GetReturnCode returns the ReturnCode field value if set, zero value otherwise.
func (o *OrderShipmentInfo200Response) GetReturnCode() int32 {
	if o == nil || IsNil(o.ReturnCode) {
		var ret int32
		return ret
	}
	return *o.ReturnCode
}

// GetReturnCodeOk returns a tuple with the ReturnCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderShipmentInfo200Response) GetReturnCodeOk() (*int32, bool) {
	if o == nil || IsNil(o.ReturnCode) {
		return nil, false
	}
	return o.ReturnCode, true
}

// HasReturnCode returns a boolean if a field has been set.
func (o *OrderShipmentInfo200Response) HasReturnCode() bool {
	if o != nil && !IsNil(o.ReturnCode) {
		return true
	}

	return false
}

// SetReturnCode gets a reference to the given int32 and assigns it to the ReturnCode field.
func (o *OrderShipmentInfo200Response) SetReturnCode(v int32) {
	o.ReturnCode = &v
}

// GetReturnMessage returns the ReturnMessage field value if set, zero value otherwise.
func (o *OrderShipmentInfo200Response) GetReturnMessage() string {
	if o == nil || IsNil(o.ReturnMessage) {
		var ret string
		return ret
	}
	return *o.ReturnMessage
}

// GetReturnMessageOk returns a tuple with the ReturnMessage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderShipmentInfo200Response) GetReturnMessageOk() (*string, bool) {
	if o == nil || IsNil(o.ReturnMessage) {
		return nil, false
	}
	return o.ReturnMessage, true
}

// HasReturnMessage returns a boolean if a field has been set.
func (o *OrderShipmentInfo200Response) HasReturnMessage() bool {
	if o != nil && !IsNil(o.ReturnMessage) {
		return true
	}

	return false
}

// SetReturnMessage gets a reference to the given string and assigns it to the ReturnMessage field.
func (o *OrderShipmentInfo200Response) SetReturnMessage(v string) {
	o.ReturnMessage = &v
}

// GetResult returns the Result field value if set, zero value otherwise.
func (o *OrderShipmentInfo200Response) GetResult() Shipment {
	if o == nil || IsNil(o.Result) {
		var ret Shipment
		return ret
	}
	return *o.Result
}

// GetResultOk returns a tuple with the Result field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderShipmentInfo200Response) GetResultOk() (*Shipment, bool) {
	if o == nil || IsNil(o.Result) {
		return nil, false
	}
	return o.Result, true
}

// HasResult returns a boolean if a field has been set.
func (o *OrderShipmentInfo200Response) HasResult() bool {
	if o != nil && !IsNil(o.Result) {
		return true
	}

	return false
}

// SetResult gets a reference to the given Shipment and assigns it to the Result field.
func (o *OrderShipmentInfo200Response) SetResult(v Shipment) {
	o.Result = &v
}

func (o OrderShipmentInfo200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o OrderShipmentInfo200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ReturnCode) {
		toSerialize["return_code"] = o.ReturnCode
	}
	if !IsNil(o.ReturnMessage) {
		toSerialize["return_message"] = o.ReturnMessage
	}
	if !IsNil(o.Result) {
		toSerialize["result"] = o.Result
	}
	return toSerialize, nil
}

type NullableOrderShipmentInfo200Response struct {
	value *OrderShipmentInfo200Response
	isSet bool
}

func (v NullableOrderShipmentInfo200Response) Get() *OrderShipmentInfo200Response {
	return v.value
}

func (v *NullableOrderShipmentInfo200Response) Set(val *OrderShipmentInfo200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableOrderShipmentInfo200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableOrderShipmentInfo200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOrderShipmentInfo200Response(val *OrderShipmentInfo200Response) *NullableOrderShipmentInfo200Response {
	return &NullableOrderShipmentInfo200Response{value: val, isSet: true}
}

func (v NullableOrderShipmentInfo200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOrderShipmentInfo200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


